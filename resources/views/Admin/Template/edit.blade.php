                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="card card-primary">
                            ...
                            <!-- /.card-header -->
                            <!-- form start -->
                            <form action="{{ route('editTemplate',$template->id) }}" enctype="multipart/form-data" method="POST">
                                @csrf
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">Days</label>
                                        <input type="number" name="days" class="form-control" id="exampleInputEmail1"
                                            placeholder="Enter Days" value="{{ $template->days }}">
                                    </div>

                                    <div class="form-group">
                                        <label for="exampleInputEmail1">Title</label>
                                        <input type="text" name="name" class="form-control" id="exampleInputEmail1"
                                            placeholder="Enter Name" value="{{ $template->title }}">
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="select2" multiple="multiple" name="excercises[]"
                                            ...
                                            <option>Select Option</option>
                                            @foreach ($excercise as $item)
                                                <option value="{{ $item->id }}" @if(in_array($item->id, [$template->excercises])) selected @endif>{{ $item->name }} -
